/**
 * Onboarding step — persist repo activations from the Day-2 picker.
 *
 ...
 * to the next step (workflows). The backend treats the payload as a
 * complete replacement set, so unticking a previously-activated repo
 * is the same code path as adding a new one.
 *
 ...
 * see, and we surface that as a wizard error banner without leaking
 * the raw id list.
 */
...
export async function POST(request: Request) {
  ...
  // ``getAll`` keeps every checkbox; the form sends one ``repo_id``
  // entry per ticked row. Empty selection is a soft error — the user
  // either wants to skip (use the link) or made a mistake.
  ...
  if (externalIds.length === 0) {
    return wizardError(origin, wsId, repo, "empty");
  }
  ...
  try {
    ...
    if (err instanceof ApiHttpError) {
      ...
      if (err.status === 409)
        return wizardError(origin, wsId, repo, "no_install");
      if (err.status === 502)
        return wizardError(origin, wsId, repo, "bad_token");
      // 422 means the user picked an id the App can't see (race with
      // an uninstall, or someone toggled access on github.com between
      // page load and submit). Funnel them back to the picker.
      if (err.status === 422)
  ...
  next.searchParams.set("step", "workflows");
...
function wizardError(origin: string, wsId: string, repo: string, code: string) {
  // Stay on the picker step so the error banner is right next to the
