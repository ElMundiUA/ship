  ...
  function onViewportPointerDown(event: ReactPointerEvent<HTMLDivElement>) {
    ...
    if (
      (event.target as Element).closest(
        "[data-process-node], [data-process-control]",
      )
    ) {
      return;
    }
    ...
    >
      <div className="pointer-events-none sticky left-0 top-0 z-20 flex h-0 justify-end p-3">
        <div
          ...
          className="pointer-events-auto flex items-center gap-1 rounded-full border border-white/10 bg-ink/85 p-1 shadow-card backdrop-blur-xl"
        >
          <button
            type="button"
