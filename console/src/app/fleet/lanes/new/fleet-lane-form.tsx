...
/**
 * Client-side "New Fleet lane" form.
 *
 * The form is a *thin* wrapper over ``POST /api/fleet-lanes`` — we
 * don't validate pattern inputs here because mirror-lane Fleet
 * lanes apply the inputs at lane-materialisation time per repo
 * (and the backend does the coerce/required check then). Cadence
 * is a free string today; validation lives where the wizard writes
 * workflows.
 */
export function NewFleetLaneForm({
  workspaceId,
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (!name.trim()) {
      setError("Give the Fleet lane a name.");
      return;
    }
    if (!laneId.trim()) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/fleet-lanes", {
        method: "POST",
      ...
      router.push("/fleet/lanes");
      router.refresh();
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Failed to create Fleet lane",
      );
    } finally {
      setSubmitting(false);
    }
    ...
    return (
      <Card>
        <CardHeader
          ...
          subtitle="Only patterns advertising 'lane' mode can back a Fleet lane. Author one with AI to get started."
          action={aiAuthor}
        />
      </Card>
    );
    ...
    <form onSubmit={submit} className="flex flex-col gap-4">
      ...
      <Card>
        ...
        <div className="mt-3 flex flex-col gap-3">
          <Labeled label="Fleet lane name" hint="Shown in the list — e.g. 'Nightly retro'">
            <input
              value={name}
      ...
      <div className="flex items-center gap-2">
        <ButtonPrimary type="submit">
          {submitting ? "Creating…" : "Create Fleet lane"}
        </ButtonPrimary>
        <ButtonGhost
          type="button"
          onClick={() => router.push("/fleet/lanes")}
        >
          Cancel
        </ButtonGhost>
      </div>
