...
update (commit a045e38) is the load-bearing fix; the server now
auto-cascades to ``dev_implementation`` up to twice before filing an
inbox escalation letter.

Behavior verified here:

- ``code_review`` / ``validation`` blocked+no_next (first pass) →
  ``cascade:blocked_no_next_auto``, no inbox row.
- After two prior auto-cascades in 4h →
  ``inbox:blocker:cascade_exhausted`` + ``needs:clarification``.
- ``dev_implementation`` blocked without stage_next stays inbox-quiet.
"""
...
async def _seed_prior_auto_cascades(
    db_session, ws_id: uuid.UUID, ticket_ref: str, count: int
) -> None:
        ...
        db_session.add(
            AuditLog(
                ...
                payload={
                    ...
                    "actions": ["cascade:blocked_no_next_auto"],
                },
...
async def test_validation_blocked_no_next_also_auto_cascades(
    ...
    """Validation is also a review-style gate — same auto-cascade applies."""
    _, raw, ws = seed_workspace
    res = await v1_client.post(
        f"/v1/workspaces/{ws.id}/agent-runs/finish",
        headers={"Authorization": f"Bearer {raw}"},
...
async def test_blocked_with_stage_next_does_not_file_letter(
    ...
    cascade matrix handles re-dispatch and no inbox row should appear."""
    _, raw, ws = seed_workspace
    res = await v1_client.post(
        f"/v1/workspaces/{ws.id}/agent-runs/finish",
        headers={"Authorization": f"Bearer {raw}"},
...
@pytest.mark.asyncio
async def test_cascade_exhausted_files_escalation_letter(
    db_session, v1_client, seed_workspace, fake_tracker
) -> None:
    """After two auto-cascades in 4h, the third blocked+no_next escalates."""
    _, raw, ws = seed_workspace
    await _seed_prior_auto_cascades(db_session, ws.id, "ELS-200", 2)
...
async def test_label_add_failure_is_swallowed_and_letter_still_lands(
    ...
    422 — the inbox row is the load-bearing signal on escalation."""
