...
update (commit a045e38) is the load-bearing fix, but the server
also auto-converts plain ``blocked+no_next`` into a structured
``needs_clarification``-shaped letter so future agents that still
ship null ``stage_next`` don't strand tickets either.

Behavior verified here:

- ``code_review`` / ``validation`` / ``auto_merge`` / ``qa_*`` /
  ``pr_review`` with ``outcome=blocked`` + no ``stage_next`` → one
  inbox row with ``intake_reason='blocked_without_cascade'`` + 3
  choice action_items.
- ``dev_implementation`` blocked without stage_next is the legacy
  transient-refire path (refire-cap handles budget); the new defense
  must NOT spam an inbox row there — keeps
  ``test_blocked_finish_does_not_create_inbox_row`` green.
"""
...
@pytest.mark.asyncio
async def test_code_review_blocked_no_next_files_blocker_letter(
    db_session, v1_client, seed_workspace, fake_tracker
) -> None:
    """Canonical case — reviewer blocks without saying who fixes it.
    Server stamps ``needs:clarification`` and files an inbox letter
    so the picker stops looping and the operator unblocks in <1 min."""
    ...
    assert "inbox:blocker:blocked_no_next" in actions
    ...
    assert {a.get("id") for a in ai} == {
        "send_to_dev", "send_to_validation", "mark_handled"
    }
...
@pytest.mark.asyncio
async def test_validation_blocked_no_next_also_files_letter(
    db_session, v1_client, seed_workspace, fake_tracker
) -> None:
    ...
    assert res.status_code == 200, res.text
    assert (
        "inbox:blocker:blocked_no_next" in (res.json().get("actions") or [])
    )
...
async def test_label_add_failure_is_swallowed_and_letter_still_lands(
    ...
    signal, the label is best-effort."""
