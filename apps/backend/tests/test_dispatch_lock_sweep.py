...
@pytest.mark.asyncio
async def test_sentry_alert_fires_only_past_90min(
    db_session, monkeypatch
) -> None:
    """ELS-151 / A4: routine 60-min sweeps must not page Sentry —
    only locks aged past 90 minutes get the breadcrumb."""
    from backend.app.services import dispatch_lock_sweep as sweep_mod
    ...
    monkeypatch.setattr(
        sweep_mod, "record_stale_project_lock_breadcrumb", _stub
    )

    ws = await _make_workspace(db_session)

    # Case A: 70-min lock (above release floor, below alert floor)
    run_a = await _insert_dispatch_audit(
        db_session, workspace_id=ws, ticket_ref="PAC-700"
    )
    await db_session.execute(
        text(
            "UPDATE audit_log SET created_at = NOW() - INTERVAL '80 minutes' "
            "WHERE id = :id"
        ),
        {"id": run_a},
    )
    await _insert_lock(
        db_session,
        workspace_id=ws,
        key="project:proj-70min",
        run_id=run_a,
        age_minutes=70,
    )

    # Case B: 120-min lock (past alert floor)
    run_b = await _insert_dispatch_audit(
        db_session, workspace_id=ws, ticket_ref="PAC-120"
    )
    await db_session.execute(
        text(
            "UPDATE audit_log SET created_at = NOW() - INTERVAL '130 minutes' "
            "WHERE id = :id"
        ),
        {"id": run_b},
    )
    ...
    # Sentry should be called exactly once — for the 120-min case
    assert len(breadcrumb_calls) == 1
    call = breadcrumb_calls[0]
    assert call["ticket_ref"] == "PAC-120"
    assert call["project_id"] == "proj-120min"
    assert call["held_minutes"] >= 120
