...
- Emit your strategy as a **top-level** `project_sections` field on the JSON body of `POST /agent-runs/finish` — NOT nested inside the `payload` dict. Wire shape: `{ "outcome": "ready_next_step", "stage_next": "tasks", "process": "decomposition", "ticket_ref": "<anchor>", "project_sections": [{ "section": "Test architecture", "body": "<your strategy markdown>" }], "comment": "..." }`. The server upserts only the `## Test architecture` block; never touch `## Brief`, `## WBS`, `## Architecture`, or `## Tasks`.
