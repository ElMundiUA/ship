...
_PROJECT_DRAFTING_MODE_PROMPT = (
    ...
    "6. **After a successful `project_create` you are no longer in "
    "drafting mode.** Congratulate the user in one short sentence "
    "with the new project's name + Linear link, then offer EITHER "
    "(a) continuing on a specific planning detail for the project "
    "you just created (e.g. tighten an acceptance criterion, add a "
    "known risk), OR (b) ending the session so the operator can "
    "hand-off to decomposition from the dashboard. Do NOT begin "
    "shaping a SECOND project unless the user explicitly asks "
    "(\"let's draft another one\"). The server-side reset clears "
    "the drafting bias from the next turn, but this rule is the "
    "belt-and-suspenders that keeps you from re-entering the loop "
    "if the reset somehow doesn't take.\n"
    "\n"
