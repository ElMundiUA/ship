...
# ``0.36`` → Drops ``process.routines`` / ``process.states`` /
#         ``process.transitions`` from the seeded ``.ship/config.yml``.
#         E16/ELS-124 moved scheduling to the backend dispatcher, so
#         the customer-side routine map is dead weight — and the
#         pre-E16 ``DEFAULT_SEED_LANES`` was still emitting the legacy
#         slugs (``intake``/``tech_arch_plan``/…) while the dispatcher
#         fires the new bundle slugs (``planning``/``developer``/…),
#         exposed on askslayer 2026-05-15 as ``unknown routine
#         'planning' in .ship/config.yml``. The seeded config now
...
def compose_seed_files(
    ...
    * ``presets`` — **deprecated, ignored.** The pre-P5-01 14-preset
    ...
    #
    # E16/ELS-124 retired the customer-side scheduler. The backend
    # dispatcher fires ``workflow_dispatch`` with the exact
    # ``(routine_id, ticket_ref)`` pair to run — customers no longer
    # own the schedule, and ``shipctl run`` no longer reads
    # ``process.routines`` to map cron → specialist. So we stop
    # writing the ``process.routines`` block: it's dead weight that
    # only causes drift bugs (e.g. pre-E16 routine slugs lingering
    # in fresh seed PRs while the new bundle dispatcher fires
    # ``--routine planning``, exposed on askslayer 2026-05-15). The
    # ``states`` / ``transitions`` map is also gone — operators read
    # the FSM definition from Ship's catalog UI now, not from a YAML
    # echo in their repo.
