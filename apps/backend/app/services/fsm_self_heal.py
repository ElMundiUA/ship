    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(30.0)) as client:
        for stage in SCAN_STAGES:
            ...
            for row in rows or []:
                ...
                # dev_not_converging detector deleted 2026-05-28 with
                # refire_cap. Its detection threshold (3 reviewer blocks
                # + 2 dev cycles in 4h) was downstream of the same
                # symptom that the new label-driven freeze now blocks
                # at the source: ``outcome=blocked`` adds the ``blocked``
                # signal label and the picker refuses the ticket on
                # every subsequent dispatch. Operator clears the label
                # to resume; no detector needed.
