...
def record_stale_project_lock_breadcrumb(
    *,
    project_id: str,
    ticket_ref: str,
    workspace_id: str,
    ...
    """Emit a Sentry breadcrumb when the lock sweeper releases a
    dangling ``project:*`` dispatch lock (ELS-151).

    Engineers triaging in Sentry see the signal where they already
    look. A4 was scoped (per ELS-153) to Sentry only — if observed
    rate exceeds ~1/month after 4-week prod soak, add a Console
    inbox row alongside this.
    """
    try:
        import sentry_sdk
        ...
        if owning_run_id:
            payload["owning_run_id"] = owning_run_id
        if project_name:
            payload["project_name"] = project_name
        sentry_sdk.add_breadcrumb(
            category="ship.lock.stale_project_lock",
            message=(
                f"Released dangling project_lock after {held_minutes} min "
                f"(project={project_name or project_id}, ticket={ticket_ref})"
            )[:300],
            level="warning",
            data=payload,
        )
        # Also capture as a low-severity message so the breadcrumb is
        # findable without an enclosing transaction — sweeper runs in
        # an APScheduler tick which doesn't wrap a Sentry transaction.
        sentry_sdk.capture_message(
            f"project_lock swept after {held_minutes}m (ticket={ticket_ref})",
            level="warning",
        )
    except Exception:  # noqa: BLE001 — observability must not break the sweeper
        log.debug(
            "sentry breadcrumb skipped for stale project lock", exc_info=True
        )
