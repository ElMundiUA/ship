/**
 * Browser-side proxy: list a workspace's activated repos.
 * GET /api/repos?ws=<workspaceId>  →  GET /v1/workspaces/{ws}/repos
 */
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const data = await listActivatedRepos(ws, token);
