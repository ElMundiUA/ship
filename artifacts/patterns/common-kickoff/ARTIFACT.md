...
- **Do not assume** a fixed prompt was embedded in the workflow YAML. Fetch the workload pattern your lane needs (for example `role-intake`, `role-tech-architect`) via `shipctl pattern fetch <id>` or the pins in `.ship/config.yml`.
