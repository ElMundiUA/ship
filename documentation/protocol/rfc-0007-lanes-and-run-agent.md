...
5. During the window, the replacement path is:
   - a reusable workflow `elmundi/ship/.github/workflows/run-agent.yml@v1`
     in this repo, and
   - thin wrappers generated by `shipctl init` from `lanes.*` in
     `.ship/config.yml`.

## Reusable workflow contract

Published from this repo at `/.github/workflows/run-agent.yml`, callable
as `uses: elmundi/ship/.github/workflows/run-agent.yml@vN`.

Inputs:

- `lane` (string, required)
- `shipctl_version` (string, default: pinned to Ship release)
- `ship_run_id`, `ship_callback_url`, `ship_run_token` (passed through).

The body is checkout → setup-node → install shipctl → `shipctl run
--lane …` → rely on `shipctl run` to call the callback. Customer-side
wrappers supply `on:`, `permissions:`, `concurrency:`, and the job input
`lane`.
...
same file into their own org via `shipctl init` — the generator emits
either a `uses:` reference (default) or an inlined copy under a named
profile (`--inline-reusable`).
...
1. **Reusable workflow availability for private orgs** — default to
   `uses:` with public `elmundi/ship` or require `--inline-reusable`
   during init? Defer decision to Phase 3 prototype.
...
- **Phase 3** — Reusable workflow + `shipctl init` wrapper generator.
