...
| **B11** | **P1 (UX)** | Dashboard counters (workspace home + per-repo home) are **hard-pinned to a 24h window** with no way to widen or narrow it. Every cutoff in `dashboard.py` (`get_ops_dashboard`, `_build_now`) and `repo_home.py` is `cutoff_24h = now - timedelta(days=1)`. There is no UI affordance to switch periods. The maintainer's framing on 2026-04-30: "от того что я буду видеть все падения за всю историю проекта мне не холодно не жарко — надо их или резолвить или ограничивать окно выборки". Today the user neither sees an actionable resolve flow nor can choose a useful window — counters just stay non-zero from one bad CI day for the next 24h, then disappear. | Add a period selector (24h / 7d / 30d / all) to `/` workspace home and `/r/<owner>/<repo>` repo home. Backend: `?window=` query param on `/dashboard/ops` and `/repo-home/...`, derives the cutoff for `failed_workflow_runs`, `recent_pipeline_runs`, `shipped_prs`, `recent_interventions`. Frontend: segment control persisted in URL (`?window=`). Default stays 24h. Pair with B3 (filter to Ship-installed workflow names) so the counter shows what the label promises. |
