...
**Приклад припущень:** сітка SDLC = **4 ролі × ~12 парних годин на добу** ⇒ **~48 тиків планувальника на добу**. Якщо лише **25%** тиків реально роблять pick і запускають агента ⇒ **~12 запусків агента на добу**. Якщо Automations зав’язані на **ширші** події (кожен перехід у **Todo** без попереднього pick), для того самого беклогу легко отримати **десятки зайвих викликів на добу**.
...
Коли issue переходить у **Todo** з `ready:developer`:
...
**Обмеження:** «Status changed → Todo» спрацьовує для будь-якого переходу в Todo — охорона в промпті через `ready:developer`.
