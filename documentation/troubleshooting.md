...
- **Likely cause:** the repo hasn't been migrated to the v2 lanes-as-config schema yet; `shipctl run` refuses to read v1.
...
- **Verify:** `shipctl config get version` prints `2`; the next `shipctl run --lane <id>` proceeds past argument parsing.

#### `shipctl run --lane <id>` exits 4 (idempotency write failure)
...
- **Verify:** after a successful run, `.ship/state/<idempotency.key>.json` exists and contains the pattern sha; re-running the lane reports `status: noop, reason: already-done`.
...
- **Verify:** the generated file starts with `# ship-cli: lanes v1 — generated by shipctl lanes install.`; `shipctl lanes list` shows the lane wired.
