...
program
    .command("init")
    .description("Initialize issue for pipeline. Features: stage:ba + ready:ba. Bugs: flow:bug + ready:bug-agent. Hotfix: --no-ba → ready:architect")
    ...
    if (dryRun) {
        const wouldAdd = opts.bug
            ? ["flow:bug", "stage:bug-agent", "ready:bug-agent"]
            : opts.noBa
                ? ["flow:no-ba", "ready:architect", "stage:architect"]
                : ["stage:ba", "ready:ba"];
        out({ dryRun: true, wouldAdd });
        return;
    }
    if (opts.bug) {
...
program
    ...
    if (role === "qa-automation" && !nextRole) {
        ...
        if (token && remote) {
            const prNumber = await findPRByIssueIdentifier(token, remote.owner, remote.repo, issue.identifier);
            if (prNumber) {
    ...
    const artifact = {
        ...
        artifacts: opts.artifacts?.split(",").map((s) => s.trim()) ?? [],
        nextRole: handoff.to,
...
program
    ...
    const handoff = computeHandoff(opts.from, opts.to, config);
    if (dryRun) {
        out({ dryRun: true, handoff });
        return;
    }
...
program
    ...
    const handoff = getBlockedHandoff(opts.role, opts.reason);
    if (dryRun) {
        out({ dryRun: true, handoff });
        return;
    }
...
program
    ...
    const handoff = getEscalatedHandoff(opts.role, opts.reason);
    if (dryRun) {
        out({ dryRun: true, handoff });
        return;
    }
...
program
    .command("ci-fail")
    .description("Return issue to Developer when CI/workflow failed (self-heal trigger)")
    .requiredOption("-i, --issue <id>", "Linear issue identifier")
    .option("--run-id <id>", "GitHub Actions workflow run ID (fetch failed job logs)")
    ...
    const remote = await getGitRemote();
    let failureText = opts.text ?? "";
    if (opts.runId && token && remote) {
        const logs = await getFailedJobLogsFromRun(token, remote.owner, remote.repo, parseInt(opts.runId, 10));
        if (logs)
            failureText += (failureText ? "\n\n" : "") + logs;
    }
    const prUrl = opts.pr && remote
        ? `https://github.com/${remote.owner}/${remote.repo}/pull/${opts.pr}`
        : "";
...
program
    ...
    .option("--autonomous", "CI mode: skip QA handoff, go straight to In Review + email (CI already ran Playwright)")
    .action(async (opts) => {
    const token = process.env.GITHUB_TOKEN;
    if (!token) {
        console.error("Missing GITHUB_TOKEN");
    ...
    if (!prNumber && opts.issue) {
        prNumber = await findPRByIssueIdentifier(token, remote.owner, remote.repo, opts.issue);
    }
    if (!prNumber) {
        console.error("Could not find PR. Use --pr <number>, run from PR branch, or ensure PR title contains issue ID (e.g. ELM-57).");
        process.exit(1);
    }
    const status = await getPRStatus(token, remote.owner, remote.repo, prNumber);
    const client = createLinearClient(apiKey);
    ...
    const previewUrl = status.previewUrl || prUrl;
    if (opts.autonomous) {
        // CI mode: pr-preview already ran Playwright. Go straight to In Review + email.
        if (!dryRun) {
        ...
        // Manual: hand off to QA (tests on preview). QA will move to In Review when done.
        if (!dryRun) {
            for (const r of ["developer", "release-manager"]) {
                try {
                    await removeLabel(client, issue.id, READY_LABELS[r]);
