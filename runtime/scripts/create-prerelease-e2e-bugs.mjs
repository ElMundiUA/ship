 ...
 * Org-specific IDs and base URL must come from .env — see documentation/examples/elmundi/index.md § Environment.
...
function requiredUuid(env, key) {
  ...
  if (!v) {
    throw new Error(
      `Missing ${key} in .env — copy UUIDs from Linear (API or UI). See documentation/examples/elmundi/index.md.`
    );
  }
  return v;
}
...
async function main() {
  ...
  if (!projectId) {
    throw new Error(
      "Set LINEAR_E2E_ISSUE_SEED_PROJECT_ID or LINEAR_SDLC_PROJECT_ID in .env (see documentation/examples/elmundi/index.md)."
    );
  }
