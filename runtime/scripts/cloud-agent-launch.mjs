...
import {
  RUNTIME_ROOT,
  PROMPTS_CLOUD_AGENT_DIR,
  gitRepoRoot,
} from "./lib/paths.mjs";

const PACKAGE_ROOT = RUNTIME_ROOT;
const DEFAULT_REPO_ROOT = process.env.REPO_ROOT || gitRepoRoot(PACKAGE_ROOT);
...
function getIssueJson(issueArg) {
  try {
    const buf = execFileSync(
      ...
      { cwd: PACKAGE_ROOT, encoding: "utf8", env: process.env, maxBuffer: 10 * 1024 * 1024 }
    );
...
async function main() {
  ...
  const roleFile = join(PROMPTS_CLOUD_AGENT_DIR, `${role}.md`);
  const baseFile = join(PROMPTS_CLOUD_AGENT_DIR, "_base.md");
