...
(``file:///…``); git URLs are accepted but require the (forthcoming) sync
worker to clone them into a local cache before the resolver can read them.
"""
...
async def create_artifact_repo(
    ...
    if not urlparse(payload.url).scheme.startswith("file") and "@" not in payload.url:
        # Pure file:// paths are read inline; everything else needs the sync
        # worker. Surface that clearly so operators don't wonder why their
        # GitHub repo doesn't appear in the resolver until the worker lands.
        repo.last_sync_error = "git sync worker not yet implemented; use file:// for now"
    session.add(repo)
    await session.flush()
