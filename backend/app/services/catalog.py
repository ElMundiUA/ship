...
def preset_bundle_files(
    ...
    *,
    ...
    lane_specs = [s for s in included_specs if s.lane_trigger is not None]
    if lane_specs:
        config_lines.append("lanes:")
        for spec in lane_specs:
            config_lines.append(f"  {spec.kind}:")
            for key, value in spec.lane_trigger.items():
                # Quote schedule crons and any value that starts with
                # a char YAML would otherwise parse funny (``*``,
                # ``{``). Simple rule: wrap strings that contain
                # shell-special characters in double quotes.
