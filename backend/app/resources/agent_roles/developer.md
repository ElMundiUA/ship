...
- After all child tickets exist, emit the index as a **top-level** `project_sections` field on the JSON body of `POST /agent-runs/finish` — NOT nested inside the `payload` dict. Wire shape: `{ "outcome": "ready_next_step", "stage_next": "planning_done", "process": "decomposition", "ticket_ref": "<anchor>", "project_sections": [{ "section": "Tasks", "body": "<list of identifiers + names>" }], "comment": "..." }`. One bullet per child ticket created. The server upserts only the `## Tasks` block.
