...
- For each line in `## WBS`, declare exactly **one child ticket** in the finish payload's top-level `child_tickets` array. The server creates each under the anchor's project (using its `create_ticket` adapter), then auto-renders a `## Tasks` section listing the freshly-created identifiers — you don't need to ship a `project_sections` entry for Tasks, and you cannot guess identifiers that don't exist yet.
