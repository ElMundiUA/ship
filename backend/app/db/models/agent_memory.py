...
class KbIndexingRun(Base):
    """One ``.ship/knowledge`` re-embedding attempt for an activated repo.

    Mirrors the :class:`KnowledgeIngestionRun` shape, but rooted on
    ``workspace_repos`` instead of ``knowledge_import_sources`` — the
    KB indexer is a per-repo job whose state vector (pending / running
    / done / error + :class:`IndexReport` counters) the Navigator's
    ``probe_repo_kb_indexing`` tool reads back out.
    """

    __tablename__ = "kb_indexing_runs"
    __table_args__ = (
        Index(
        ...
        Index("ix_kb_indexing_runs_workspace", "workspace_id"),
        # Partial index over in-flight rows: probe asks "is *anything*
        # currently queued / running for this repo?" on every call, and
        # the steady state has that set near-empty.
        Index(
