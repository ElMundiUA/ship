"""HTTP tests for workspace Fleet lanes (RFC-0008 §G — PR-5).

Covers the mirror-lane MVP end-to-end: create the Fleet lane, list
it with a compliance rollup that correctly buckets activated repos
into ``compliant`` / ``missing`` / ``excepted``, toggle exceptions,
and delete. Pattern validation (unknown pattern / request-only
pattern) is also pinned because the Console's Fleet-lane form hits
these paths before ever touching the DB.

Naming history: this used to be ``test_v1_policies.py`` exercising
``/workspaces/{ws}/policies``. The endpoint moved to ``/fleet-lanes``
to free up the "policy" name for free-text standing rules.
"""
...
async def seed_fleet_lane_repos(db_session, seed_workspace):
    ...
    # Wire the Fleet-lane-matching lane on repo_a so the compliance
...
@pytest.mark.asyncio
async def test_create_and_list_fleet_lane_with_compliance(
    v1_client, seed_fleet_lane_repos
) -> None:
    raw, workspace, repo_a, repo_b = seed_fleet_lane_repos
    headers = {"Authorization": f"Bearer {raw}"}

    create_resp = await v1_client.post(
        f"/v1/workspaces/{workspace.id}/fleet-lanes",
        headers=headers,
        json={
            "name": "Nightly retro",
    ...
    list_resp = await v1_client.get(
        f"/v1/workspaces/{workspace.id}/fleet-lanes", headers=headers
    )
    assert list_resp.status_code == 200, list_resp.text
    assert len(list_resp.json()["fleet_lanes"]) == 1


@pytest.mark.asyncio
async def test_duplicate_lane_id_rejects(
    v1_client, seed_fleet_lane_repos
) -> None:
    raw, workspace, _, _ = seed_fleet_lane_repos
    headers = {"Authorization": f"Bearer {raw}"}
    ...
    assert dup.json()["detail"]["code"] == "fleet_lane_conflict"
...
@pytest.mark.asyncio
async def test_unknown_pattern_404(v1_client, seed_fleet_lane_repos) -> None:
    raw, workspace, _, _ = seed_fleet_lane_repos
    resp = await v1_client.post(
        f"/v1/workspaces/{workspace.id}/fleet-lanes",
        headers={"Authorization": f"Bearer {raw}"},
...
async def test_exception_toggle_moves_repo_bucket(
    ...
    raw, workspace, _, repo_b = seed_fleet_lane_repos
    headers = {"Authorization": f"Bearer {raw}"}
    ...
    fleet_lane_id = create.json()["id"]

    add_exc = await v1_client.post(
        f"/v1/workspaces/{workspace.id}/fleet-lanes/{fleet_lane_id}/exceptions/{repo_b.id}",
        headers=headers,
...
@pytest.mark.asyncio
async def test_delete_fleet_lane(
    v1_client, seed_fleet_lane_repos
) -> None:
    raw, workspace, _, _ = seed_fleet_lane_repos
    headers = {"Authorization": f"Bearer {raw}"}
    ...
    delete = await v1_client.delete(
        f"/v1/workspaces/{workspace.id}/fleet-lanes/{fleet_lane_id}",
        headers=headers,
    )
    ...
    assert listing.json()["fleet_lanes"] == []
