...
The Node CLI at **`dist/cli.js`** orchestrates multi-role workflow commands (`next`, `start`, `complete`, `release-check`, `pr-create`, …) against an **issue tracker**. The npm package is **`ship-agent`**; it still exposes a **`linear-agent`** bin name for backward compatibility with scripts and muscle memory.
...
- **Role labels** from `dist/agent-contracts.js` in the package — e.g. `ready:developer`, `stage:ba`, `flow:no-ba`, `result:failed`. Automation and docs assume these **exact strings** (as labels, tags, or GitHub labels depending on the system).
...
**Scope note:** Pick scripts under `scripts/` and the reference GitHub workflows in this repo are still **Linear-oriented** (project IDs, GraphQL). The CLI multi-tracker work lets you run **`node dist/cli.js …`** locally or from custom workflows against Jira, GitHub Issues, Azure Boards, or ClickUp without changing command names.
...
- Run: `node dist/cli.js <command>` from the Ship package root, or **`npx ship-agent`** / **`ship-agent`** after install.
- **`linear-agent`** remains a **bin alias** pointing at the same `dist/cli.js`.
