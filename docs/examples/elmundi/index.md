...
!!! note "Ship log"
    Ukrainian runbooks for this chapter live beside the English source as `index.uk.md` and siblings under `docs/examples/elmundi/` (see `mkdocs.yml` `exclude_docs` if your local build hides them). They are the human translation layer; the tables below stay canonical for automation.
...
| **Deterministic pick** | Node scripts under `scripts/` (Ship repo root; in **elmundi**: `tools/linear-agent/scripts/`) — invoked from workflows; details in SDLC + operator docs. |
| **Versioned prompts** | `cloud-prompts/*.md` — one role per file; see [Prompt catalog](../../prompts-workflows/index.md#prompt-catalog). |
...
Stages **intake → dev** run from **`.github/workflows/linear-agent-sdlc-scheduled.yml`**. Cursor Automations are **off** for these stages in this deployment. GitHub **schedules** runs or accepts **`workflow_dispatch`**; role work runs on **Cursor Cloud Agent** via `cloud-agent-launch.mjs`.
...
| Secret | Purpose |
|--------|---------|
| `LINEAR_API_KEY` | Pick scripts, `dist/cli.js start`, agent comments |
...
|-----|-------------|--------------|-------|
| intake | `scripts/pick-intake-issue.mjs` | `cloud-prompts/intake.md` | Only if an issue was picked |
| clarification | `scripts/pick-clarification-issue.mjs` | `cloud-prompts/clarification.md` | Only if picked |
| ba | `scripts/pick-ba-issue.mjs` | `cloud-prompts/ba.md` | Only if picked |
| developer | `scripts/pick-next-dev-issue.mjs` | `cloud-prompts/developer.md` | After `cli start` → In Progress |
...
2. **Queues:** `scripts/agent-queue-snapshot.mjs` — counts per pick pool and next candidate.
   ```bash
node scripts/agent-queue-snapshot.mjs
...
```bash
node scripts/sync-linear-team-labels.mjs
## --dry-run — plan only
```
...
```bash
node scripts/ensure-audit-linear-projects.mjs
## plan only: ... --dry-run
```
...
Prompts: `cloud-prompts/tech-architect.md`, `qa-architect.md`, `security-officer.md`.
...
```bash
node scripts/create-prerelease-e2e-bugs.mjs
```
...
3. `cloud-prompts/developer.md` + `_base.md` — branch `fix/ELM-XX-auto`, anti-duplicate PR.
4. Local: `bash scripts/verify-setup.sh` (from Ship package root).
...
| Step | Command |
|------|---------|
| 1. Verify env | `bash scripts/verify-setup.sh` |
| 2. Verify ticket | `bash scripts/run-ticket-verify.sh ELM-XX` |
| 3. Dry-run (no changes) | `bash scripts/run-autonomous-local.sh ELM-XX --verify-only` |
...
| **`--no-agent`** | Run `node dist/cli.js pr-create -i ELM-XX` manually after you push |
...
Checks: Linear API, GitHub token, Cursor API, Bunny (optional), linear-agent dist.
...
node dist/cli.js get ELM-XX
bash scripts/run-autonomous-local.sh ELM-XX --no-agent
bash scripts/run-autonomous-local.sh ELM-XX --cloud
```
...
| **Issue metadata** | `dist/cli.js get` / Linear API via pick scripts and CLI |
...
| **Bias & output variance** | Same **versioned** prompt bundle from `cloud-prompts/` + `.cursor/skills`; changes go through PR review. | Prompts live in the **Automation UI**; drift across automations, harder to diff/review like code; **LLM bias / anchoring** applies to both stacks, but **governance** (review, tests, PR) is tighter when prompts are in-repo. |
