...
Виконання прив’язане до **GitHub Actions** у репозиторії **elmundi**. **Cursor Cloud Agent** працює в хмарі Cursor проти клонованого репо; **Linear** отримує оновлення через API, коли є ключі.
...
| Проєкт | Роль |
|--------|------|
| **ElMundi pre-release** | Операційний SDLC. Автоматизація **не** обирає з **Backlog**; issues мають бути в **Todo** з pick-фільтрами (проєкт + мітки). |
| **ElMundi tech debt** | Висновки tech architect і QA architect (на основі доказів). |
| **ElMundi security** | Залежності/безпека з Snyk (дедупліковано). |
